...
def _compute_erm_proj(p, subj, projs, kind, bad_file, remove_existing=False,
    ...
    _overwrite(write_proj, cont_proj, pr)
    return pr
    ...
    for si, subj in enumerate(subjects):
        ...
        if any(proj_nums[0]):
            ...
            if len(ecg_epochs) >= 20:
                _overwrite(write_events, ecg_eve, ecg_epochs.events)
                ...
                _overwrite(write_proj, ecg_proj, pr)
                projs.extend(pr)
            else:
        ...
        # save the projectors
        _overwrite(write_proj, all_proj, projs)
...
def _compute_add_eog(p, subj, raw_orig, projs, eog_nums, kind, pca_dir,
    ...
    if eog_nums.any():
        ...
        del eog_events
        if len(eog_epochs) >= 5:
            _overwrite(write_events, eog_eve, eog_epochs.events)
            ...
            _overwrite(write_proj, eog_proj, pr)
            projs.extend(pr)
        else:
