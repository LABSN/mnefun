...
# Generate events
mne_process_raw --raw test_raw_sss.fif --eventsout test-eve.fif

# Averaging no filter
mne_process_raw --raw test_raw_sss.fif --projon --filteroff \
        --saveavetag -nf-ave --ave test-no-reject.ave

# Averaging 40Hz
mne_process_raw --raw test_raw_sss.fif --lowpass 40 --projoff \
        --saveavetag -ave --ave test.ave

# Compute the noise covariance matrix
mne_process_raw --raw test_raw_sss.fif --filteroff --projon \
...
# Compute projection
mne_process_raw --raw test_raw_sss.fif --events test-eve.fif --makeproj \
